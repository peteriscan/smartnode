...
// Constants
const (
	erigonTagProd          string = "thorax/erigon:v2.48.1"
	erigonTagTest          string = "thorax/erigon:v2.48.1"
	erigonEventLogInterval int    = 1000
	erigonStopSignal       string = "SIGTERM"
)
...
func NewErigonConfig(cfg *RocketPoolConfig) *ErigonConfig {
	return &ErigonConfig{
		...
		CacheSize: config.Parameter{
			...
			Default:              map[config.Network]interface{}{config.Network_All: calculateErigonCache()},
		...
		MaxPeers: config.Parameter{
			ID:                   "maxPeers",
			Name:                 "Max Peers",
			Description:          "The maximum number of peers Erigon should connect to. This can be lowered to improve performance on low-power systems or constrained config.Networks. We recommend keeping it at 12 or higher.",
